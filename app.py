...
from utils.imagemap import get_new_image
from utils.message import get_coffee_roasting,get_coffee_flavor,get_africa,get_asia,get_latin
...
def reply_richmenu(get_message,reply_token):
...
def reply_coffee_roasting(get_message,reply_token):    
    reply=TextSendMessage(text=get_message)
...
def reply_coffee_flavor(get_message,reply_token):
    reply=TextSendMessage(text=get_message)
    #品種
...
def handle_message(event):
    ...
    if get_message in json_file["rich_menu"]:
        reply_richmenu(get_message,reply_token)
    elif get_message in json_file["coffee_roasting"]:
        reply_coffee_roasting(get_message,reply_token)
    elif get_message in json_file["coffee_flavor"]:
        reply_coffee_flavor(get_message,reply_token)
